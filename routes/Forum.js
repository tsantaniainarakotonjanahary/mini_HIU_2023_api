...
router.post("/", auth, async function (req, res, next) {
  ...
  try {
    ...
    const etudiant = await db
      .collection("etudiant")
      .findOne({ _id: new ObjectId(etudiantId) });
    ...
    res
      .status(201)
      .json({
        program: insertedforum,
        message: "Publication forum créée avec succès.",
      });
  } catch (error) {
    console.error(error);
    res
      ...
      .json({
        message:
          "Une erreur est survenue lors de la création de la publication forum.",
      });
  } finally {
    client.close();
  }
...
router.post(
  "/:id/commentaires/:idCommentaire/reponses",
  auth,
  async function (req, res, next) {
    ...
    const result = await db.collection("forum").updateOne(
      {
        _id: new ObjectId(publicationId),
        "commentaire.idCommentaire": new ObjectId(commentaireId),
      },
      {
        $push: {
          "commentaire.$.reponse": {
    ...
    if (result.modifiedCount === 0) {
      res
        .status(404)
        .json({ message: "Publication ou commentaire introuvable" });
      return;
    }
...
router.get("/:forumId", async function (req, res, next) {
  ...
  try {
    await client.connect();
    const db = client.db("hiu");
    const forums = await db.collection("forum").findOne({_id: new ObjectId(forumId)})
    res.status(200).json(forums);
  } catch (error) {
    console.error(error);
    res
...
router.delete("/:id", auth, async function (req, res, next) {
  ...
  try {
    ...
    const result = await db
      .collection("forum")
      .deleteOne({ _id: new ObjectId(publicationId) });
...
router.delete(
  "/:id/commentaires/:idCommentaire/reponses/:idReponse",
  auth,
  async function (req, res, next) {
    const etudiantId = req.user.id;
    const publicationId = req.params.id;
    ...
    try {
      ...
      const result = await db
        ...
        .updateOne(
          ...
          {
            $pull: {
              "commentaire.$.reponse": { idReponse: new ObjectId(reponseId) },
            },
          }
        );
      if (result.modifiedCount === 0) {
        res
          .status(404)
          .json({ message: "Publication, commentaire ou réponse introuvable" });
        return;
      }
